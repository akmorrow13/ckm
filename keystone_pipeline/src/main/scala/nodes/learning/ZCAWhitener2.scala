...
class ZCAWhitener2(val whitener: DenseMatrix[Double], val means: DenseVector[Double])
...
class ZCAWhitenerEstimator2(val eps: Double = 0.1)
  ...
  def fit(in: RDD[DenseMatrix[Double]]): ZCAWhitener2 = {
    fitSingle(in.first)
  }

  def fitSingle(in: DenseMatrix[Double]): ZCAWhitener2 = {
    val means = (mean(in(::, *))).toDenseVector

    val whitener: DenseMatrix[Double] = {
      val inc = convert(in, Float)
      val rows = inc.rows
      val cols = inc.cols
    ...
    new ZCAWhitener2(whitener, means)
